...
from HTMLReport.src.tools.template import TemplateMixin, ResultStatus
...
class TestRunner(TemplateMixin, TestSuite):
    ...
    def run(self, test: TestSuite, debug: bool = False, threadSuite=0):
        ...
        if self.sequential_execution:
            ...
            if threadSuite:
                for suite in test:
                    __result = Result(self.LANG, self.tries, self.delay, self.back_off, self.max_delay, self.retry,
                                      self.thread_start_wait, self.failed_image)
                    suite.run()
                    time.sleep(1)
